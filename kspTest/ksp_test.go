...
func TestGetData(t *testing.T) {
	...
	// 输出 IP 地址映射
	fmt.Println("IP Mapping:")
	for i, ip := range ipList {
		fmt.Println(reverseIpMap[i])
		fmt.Printf("%d: %s\n", i, ip)
	}
	...
	for i := 0; i < len(matrix); i++ {
		for j := 0; j < len(matrix); j++ {
			fmt.Printf("%d ", matrix[i][j]) // 打印int类型的矩阵值
		}
		fmt.Println()
	}
}
func TestPrintKShortestPaths(t *testing.T) {
	...
	for len(allPaths) < k && iterations < 3 {
		...
		fmt.Println("Flow matrix after updating capacity:")
		for i := 0; i < graph.Nodes; i++ {
			fmt.Println(graph.Flow[i])
		}
		iterations++
...
func TestGetPaths(t *testing.T) {
	paths := GetPaths(3, 2, 4, 1, "192.168.0.1", "192.168.2.1")
	// 输出每条路径
	fmt.Println("Retrieved IP Paths:")
